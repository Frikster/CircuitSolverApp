...
/**Processes the wire after a first detection to normalize them.
 * Created by Simon on 21.11.2016.
 */

public class WireProcessing {
    private List<List<Element>> wiresToProcess;
    private List<Component> objectizedComponents;
    public WireProcessing(List<List<Element>> wires, List<Component> components){
        wiresToProcess = new ArrayList<>(wires);
        objectizedComponents = new ArrayList<>(components);
    }

    public List<List<Element>> processWires(){
        wiresToProcess = separateComponents(wiresToProcess);
        wiresToProcess = completeMissingEndings(wiresToProcess, thresholdXY, distanceFromComponent);
        wiresToProcess = addOrphansToWires(wiresToProcess, objectizedComponents, distanceFromComponent);
        wiresToProcess = addMisingWires(wiresToProcess,findCornersToWire(wiresToProcess));
        wiresToProcess = removeDuplicateWires(wiresToProcess);

        return wiresToProcess;
    }
    ...
    /**Add the non detected components to the wires
     *
